using UnityEngine;

public class CameraController : MonoBehaviour
{
    Vector3 diff; //ターゲットとの距離の差
    GameObject player; //ターゲットとなるプレイヤー情報

    public float followSpeed = 8; //カメラの補完スピード


    //カメラの初期位置
    public Vector3 defaultPos = new Vector3(0, 6, -6);
    public Vector3 defaltRotate = new Vector3(12, 0, 0);


    // Start is called once before the first execution of Update after the MonoBehaviour is created
    void Start()
    {
        //カメラを変数で決めた初期位置・角度にする
        transform.position = defaultPos;
        transform.rotation = Quaternion.Euler(defaltRotate); //RotationはQuaternion型

        //プレイヤー情報の取得
        player = GameObject.FindGameObjectWithTag("Player");

        //プレイヤーとカメラの距離感を記憶しておく
        diff = player.transform.position - transform.position;
    }

    // Update is called once per frame
    void Update()
    {
        //プレイヤーがみつからなければ何もしない
        if (player == null)
            return;

        //線形保管を使って、カメラを目的の場所に動かす
        //Lerpメソッド(今の位置、ゴールトすべき位置、割合)
        transform.position = Vector3.Lerp(transform.position, player.transform.position - diff, followSpeed * Time.deltaTime);
        
    }
}
